package ua.nure.kn156.doroshenko.agent;
...
public class RequestServer extends CyclicBehaviour {
	...
	private ACLMessage createReply(ACLMessage message) {
		...
		if(tokenizer.countTokens()==2){
			...
			try{
				users = (Collection<User>)DaoFactory.getInstanse().getUserDao().find(firstName, lastName);
			...
			StringBuffer buffer = new StringBuffer();
			 for (User user: users) {
			    ...
			    buffer.append(user.getLasttName()).append(",");
			}
